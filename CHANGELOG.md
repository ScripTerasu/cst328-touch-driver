...
- **Validated on real hardware**: flashed `examples/waveshare-esp32s3-touch-lcd-2p8` to an actual Waveshare ESP32-S3-Touch-LCD-2.8 board (2026-08-26). `reset()`, `get_attribute()` (including the `0xCACA` firmware-CRC check), and `touches()` (including the new `0xAB` frame-marker validation, real per-point pressure, and two simultaneous touch points) all worked correctly over a multi-hundred-report session with zero I2C errors or panics. See [README.md#hardware-notes](README.md#hardware-notes) for what's still unverified (most `RunMode` variants beyond `Normal`/`DebugInfo`, and CST3530 specifically).
...
At the time of this commit, not yet validated against real hardware — see the `Unreleased` section above for the hardware validation that followed, and [README.md#hardware-notes](README.md#hardware-notes) for current status.
