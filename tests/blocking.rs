...
#[test]
fn get_attribute_rejects_bad_fw_crc() {
...
#[test]
fn set_mode_writes_normal_mode_register() {
    let expectations = [I2cTransaction::write(ADDR, REG_NORMAL_MODE_BYTES.to_vec())];
    let mut i2c = I2cMock::new(&expectations);

    let mut driver = CST328::new(i2c.clone(), NoopDelay::new());
    driver.set_mode(RunMode::Normal).unwrap();

    i2c.done();
}

#[test]
fn set_mode_writes_debug_info_mode_register() {
    ...
    let mut driver = CST328::new(i2c.clone(), NoopDelay::new());
    driver.set_mode(RunMode::DebugInfo).unwrap();
